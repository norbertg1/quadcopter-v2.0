...
void initTimer0()		//TPM1 Timer for PID
{
	SIM_SCGC6 |= SIM_SCGC6_TPM1_MASK;		//Clock enable
	TPM1_SC = 0;							//Zero registers
	TPM1_CONF = 0;
    TPM1_SC = TPM_SC_PS(TPM0_CLK_PRESCALE);	//Set the prescler
    TPM1_MOD = TPM0_CLOCK/(1<<TPM0_CLK_PRESCALE)/PID_frequency;	//Set the frequency of Timer/PWM
    TPM1_SC |= TPM_SC_CMOD(1);	//Enable the counter and interrupts
}

void initTimer1()		//TPM2 Timer for SDcard save
{
	SIM_SCGC6 |=  SIM_SCGC6_TPM2_MASK;		//Clock enable
    TPM2_SC = 0;							//Zero registers
    TPM2_CONF = 0;
    TPM2_SC = TPM_SC_PS(TPM0_CLK_PRESCALE);	//Set the prescaler
    TPM2_MOD = TPM0_CLOCK/(1<<TPM0_CLK_PRESCALE)/LOG_frequency;	//Set the frequency of Timer/PWM
