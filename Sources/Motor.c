...
void SetMotorPWM(int MotorA , int MotorB, int MotorC, int MotorD)
{
	...
	TPM0_C0V = ELEMENTARY_PROC_TIME*((ESC_MINIMUM_POWER+(ESC_POWER_DIFFERENCE)*((float)MotorD/1000))/1000);
	TPM0_C2V = ELEMENTARY_PROC_TIME*((ESC_MINIMUM_POWER+(ESC_POWER_DIFFERENCE)*((float)MotorB/1000))/1000);
	TPM0_C3V = ELEMENTARY_PROC_TIME*((ESC_MINIMUM_POWER+(ESC_POWER_DIFFERENCE)*((float)MotorA/1000))/1000);
	TPM0_C4V = ELEMENTARY_PROC_TIME*((ESC_MINIMUM_POWER+(ESC_POWER_DIFFERENCE)*((float)MotorC/1000))/1000);
#else
	TPM0_C1V = 0;
	TPM0_C2V = 0;
	TPM0_C0V = 0;
