...
void InitUARTs()
{
	...
	PORTC_PCR3 = PORT_PCR_MUX(3) | PORT_PCR_DSE_MASK;   //UART1_RX
	PORTC_PCR4 = PORT_PCR_MUX(3) | PORT_PCR_DSE_MASK;  	//UART1_TX

	PORTD_PCR2 = PORT_PCR_MUX(3) | PORT_PCR_DSE_MASK;   //UART2_RX
	PORTD_PCR3 = PORT_PCR_MUX(3) | PORT_PCR_DSE_MASK;  	//UART2_TX
	...
	//We have to feed this function the clock in KHz!
     uart1_init (CORE_CLOCK/2/1000, SDA_SERIAL_BAUD);
     uart2_init (CORE_CLOCK/2/1000, SDA_SERIAL_BAUD);
...
/*    SIM_SCGC4 |= SIM_SCGC4_UART1_MASK;
  ...
  */
	...
	uint16_t divisor = (PERIPHERAL_BUS_CLOCK / OVER_SAMPLE) / 115200;
    ...
    UART2_C4 = UARTLP_C4_OSR(OVER_SAMPLE - 1);
    UART2_BDH = (divisor >> 8) & UARTLP_BDH_SBR_MASK;
    UART2_BDL = (divisor & UARTLP_BDL_SBR_MASK);
...
void uart1_init (int sysclk, int baud)
{
   ...
   //         UART1_C5|= UART_C5_BOTHEDGE_MASK;
        
        // Setup OSR value 
        reg_temp = UART1_C4;
//        reg_temp &= ~UART_C4_OSR_MASK;
//        reg_temp |= UART1_C4_OSR(osr_val-1);
    
        // Write reg_temp to C4 register
        UART1_C4 = reg_temp;
        
       reg_temp = (reg_temp /*& UART_C4_OSR_MASK*/) + 1;
        sbr_val = (uint32)((uart0clk)/(baud_rate * (reg_temp)));
        
         /* Save off the current value of the uartx_BDH except for the SBR field */
        reg_temp = UART1_BDH & ~(UART_BDH_SBR(0x1F));
   
        UART1_BDH = reg_temp |  UART_BDH_SBR(((sbr_val & 0x1F00) >> 8));
...
//        reg_temp |= UART1_C4_OSR(osr_val-1);
       ...
       reg_temp = (reg_temp /*& UART_C4_OSR_MASK*/) + 1;
        ...
        UART2_BDH = reg_temp |  UART_BDH_SBR(((sbr_val & 0x1F00) >> 8));
        UART2_BDL = (uint8)(sbr_val & UART_BDL_SBR_MASK);
